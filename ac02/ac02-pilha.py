...
Modifique a função pilha_letras, que você fez acima, para que ela registre o
estado da pilha após cada uma das inserções.
...
Por exemplo, "(" encaixa com ")", mas não encaixa
com "]", "}", nem "(", nem "["

Por exemplo, "[" encaixa com "]", mas não encaixa
com ")", "}", nem "(", nem "["

Na verdade, "(" só encaixa com ")",
"[" só encaixa com "]" e "{" só encaixa com "}"

Faça uma função encaixa, que recebe duas strings, um
"abre" e um "fecha", e retorna
        ...
        if c in '([{<':
            ...
            if len(pilha) == 0:
                ...
                if not pilha_vazia(pilha):
                    if encaixa(pilha[-1], c):
                        pilha.pop()

    return not len(pilha) > 0
